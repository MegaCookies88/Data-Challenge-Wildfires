...
# Fitting the model
mBA_lasso <- glmnet(log1p(BA) ~ . -CNT -clim1 -clim2, 
                    family = gaussian(),
                    alpha = 1,
                    lambda = bestlam,
                    data = train)
...
# Prediction 
# not possible in glmnet to get se.fit so no distribution function
# Funny comment on some forum for glmnet : "I feel like the only people who would request SE's from a penalized regression model are the same type of people who would grossly misinterpret them."
...
```{r echo=FALSE}
# lc2+lc3+lc4+lc8+lc11+lc12+lc14+lc17+lc18
# Activate Multi-Threads and start clock
start.time <- Sys.time()
cl <- makeCluster(6)

# Fitting the model
BAm1s <- bam(log1p(BA) ~ -1 + year + month + s(lon,lat,bs="gp") + s(area,bs="cr",k=12)
             + s(altiMean,bs="cr",k=12) + s(wind,bs="cr",k=12) + s(RH,bs="cr",k=12)
             + s(clim4,bs="cc",k=7)
             + ti(lc12,wind,k=12) + ti(lc11,wind,k=20)
             + s(lc18),
             data=train,
             family=gaussian(),
             discrete=FALSE,
...
cat("\n\nTime to fit the model :", elapsed.time, "s")
